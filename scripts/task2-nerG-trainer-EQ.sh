...
for pretrained in "${!models[@]}"; do
  run_version="${models[$pretrained]}-MR_EQ"
  ...
  for dataset in "${datasets[@]}"; do
    ...
    python -m deepspeed.launcher.runner --include="localhost:$CUDA_VISIBLE_DEVICES" --master_port $port \
      task2-nerG-trainer.py "data/gner/each-EQ/${dataset}-train-EQ.jsonl" \
      --eval_file "data/gner/each-sampled-EQ/${dataset}-dev=100-EQ.jsonl" \
