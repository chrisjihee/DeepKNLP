...
def val_loop(
    ...
    metrics: Mapping[str, Any] = {
        "step": round(fabric.all_gather(torch.tensor(model.args.prog.global_step * 1.0)).mean().item()),
        "epoch": round(fabric.all_gather(torch.tensor(model.args.prog.global_epoch)).mean().item(), 4),
        "val_loss": fabric.all_gather(torch.stack(loss_list)).mean().item(),
        "val_acc": fabric.all_gather(torch.stack(acc_list)).mean().item(),
    }
    fabric.log_dict(metrics=metrics, step=metrics["step"])
    fabric.print(f"(Ep {model.args.prog.global_epoch:4.2f}) {progress}"
...
def test_loop(
    ...
    metrics: Mapping[str, Any] = {
        "step": round(fabric.all_gather(torch.tensor(model.args.prog.global_step * 1.0)).mean().item()),
        "epoch": round(fabric.all_gather(torch.tensor(model.args.prog.global_epoch)).mean().item(), 4),
        "test_loss": fabric.all_gather(torch.stack(loss_list)).mean().item(),
        "test_acc": fabric.all_gather(torch.stack(acc_list)).mean().item(),
    }
    fabric.log_dict(metrics=metrics, step=metrics["step"])
    fabric.print(f"(Ep {model.args.prog.global_epoch:4.2f}) {progress}"
...
def train(
        ...
        train_file: str = typer.Option(default="KorQuAD_v1.0_train.json"),
        valid_file: str = typer.Option(default="KorQuAD_v1.0_dev.json"),
        ...
        train_batch: int = typer.Option(default=16),
